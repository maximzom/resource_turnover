...
public class OrderService {
    ...
    @Transactional
    public Order createOrder(Long resourceId, BigDecimal quantity, LocalDate deliveryDate) {
    ...
    public Order assignSupplier(Long orderId, Long supplierId) {
        ...
        if (order.getStatus() != OrderStatus.PENDING_SUPPLY && order.getStatus() != OrderStatus.PENDING_EXECUTION) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                    "Only PENDING_SUPPLY or PENDING_EXECUTION orders can be assigned");
        }
    ...
    private void handleSpecialStatusCases(Order order, OrderStatus current, OrderStatus newStatus) {
        if ((current == OrderStatus.PENDING_EXECUTION || current == OrderStatus.IN_DELIVERY) && newStatus == OrderStatus.CANCELLED) {
            Resource resource = order.getResource();
            resource.setQuantity(resource.getQuantity().add(order.getQuantity()));
            resourceRepository.save(resource);
        }
